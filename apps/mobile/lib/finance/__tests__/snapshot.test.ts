...
describe('currency-scoped chat snapshot', () => {
  it('never combines metrics from different currencies', () => {
    const snapshot = buildFinanceAssistantToolSnapshotByCurrency([
      { amountMinor: decimalToMinor('10'), currency: 'USD', type: 'expense', analysisExcluded: false, categoryId: null, merchant: null, transactionDate: new Date().toISOString() },
      { amountMinor: decimalToMinor('20'), currency: 'SGD', type: 'expense', analysisExcluded: false, categoryId: null, merchant: null, transactionDate: new Date().toISOString() },
    ], {}, []);
