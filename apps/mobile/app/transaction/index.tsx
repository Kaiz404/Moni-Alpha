...
export default function TransactionsScreen() {
  ...
  const currency = Array.isArray(params.currency)
    ? params.currency[0]
    : params.currency;
  const month = Array.isArray(params.month)
    ? params.month[0]
    : params.month;
  const rows = useValue(
    transactions$({
  ...
  const selectedWallet = useValue(
    walletId ? walletById$(walletId) : walletById$(''),
  );
  const selectedBalance = useValue(
    walletId
    ...
    () =>
      rows.reduce(
        (all, row) => ({
          income:
            all.income +
            (row.type === 'income' ? Number(row.amountMinor) : 0),
          expense:
            all.expense +
            (row.type === 'expense' ? Number(row.amountMinor) : 0),
        }),
        { income: 0, expense: 0 },
      ),
    [rows],
      ...
      >
        ...
        <View className="mb-2 flex-row items-center justify-between">
          ...
          <Link
            href={
              walletId
                ? (`/transaction/new?walletId=${walletId}` as any)
              ...
              >
                <View className="flex-row justify-between">
                  <View className="flex-1 pr-2">
                    <Text className="font-semibold text-foreground">
                      {transfer
                        ? `${walletNames[row.walletId] ?? 'Wallet'} → ${walletNames[row.transferToWalletId ?? ''] ?? 'Wallet'}`
                        : (row.merchant ??
                          row.description ??
                          row.type)}
                    </Text>
                    <Text className="mt-1 text-xs text-muted">
                      ...
                      {row.categoryId
                        ? ` · ${categories[row.categoryId] ?? 'Uncategorized'}`
                        : ''}
                    </Text>
                  </View>
                  <View className="items-end">
                    ...
                    <Text
                      className={`mt-1 font-bold ${transfer ? 'text-transfer' : row.type === 'income' ? 'text-income' : 'text-expense'}`}
                    >
                      {sign}
                      {formatMinorAmount(
                        row.amountMinor,
