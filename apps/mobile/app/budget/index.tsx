...
export default function BudgetsScreen() {
  ...
  const currencies = useMemo(
    () =>
      [...new Set([...wallets.map((wallet) => wallet.currency), ...progress.map((row) => row.currency)])]
        .sort(),
    [progress, wallets],
  );
  const grouped = useMemo(
    () =>
      Object.entries(
        progress.reduce<Record<string, typeof progress>>((all, row) => {
  ...
  const save = async (categoryId: string, currency: string) => {
    ...
    try {
      ...
      else await upsertCategoryBudget(categoryId, currency, parseAmountInput(amount));
    } catch (error) {
      Alert.alert(
      ...
      >
        <View className="flex-row items-start justify-between gap-4">
          <View className="flex-1">
            <Text className="text-2xl font-bold text-foreground">This month</Text>
            <Text className="mt-2 text-[15px] leading-5 text-muted">
              Spending is kept separate by currency. Debt activity and wallet
              transfers do not count here.
            </Text>
          </View>
          <IconAction
            accessibilityLabel={managing ? 'Finish managing budgets' : 'Manage budgets'}
        ...
        {managing ? (
          <View className="mt-7">
            <Text className="text-base font-bold text-foreground">Set monthly caps</Text>
            <Text className="mt-1 text-sm leading-5 text-muted">
              Leave a cap empty to remove it. A cap applies only to the shown
              currency.
            </Text>
            {currencies.length === 0 ? (
              <FeedbackState
              ...
              currencies.map((currency) => (
                <View key={currency} className="mt-6">
                  ...
                  {categories.map((category) => {
                    ...
                    return (
                      <Surface key={id} className="mb-3 p-4">
          ...
          grouped.map(([currency, rows]) => (
            <View key={currency} className="mt-8">
              <Text className="mb-3 text-lg font-bold text-foreground">{currency}</Text>
              {rows.map((row) => {
                      ...
                      <View className="flex-row items-start justify-between gap-3">
                        <View className="min-w-0 flex-1 flex-row items-center gap-3">
                          <View className="h-10 w-10 items-center justify-center rounded-full bg-primary-muted">
                            <Text className="text-lg">{row.categoryIcon ?? '•'}</Text>
                          </View>
