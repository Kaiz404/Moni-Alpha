...
export default function DebtsScreen() {
  ...
  const totals = useMemo(() => {
    ...
    for (const { debt, balanceMinor } of shown)
      map.set(
        debt.currency,
        (map.get(debt.currency) ?? 0) + Number(balanceMinor),
      );
    ...
    <ScreenShell>
      ...
      >
        ...
        {totals.map(([currency, amountMinor]) => (
          ...
          >
            {formatMinorAmount(amountMinor, currency)} outstanding
          </Text>
        ))}
        {shown.length === 0 ? (
          <View className="mt-12">
            <FinanceState
              title="No debts here"
              detail="Track money you lend or borrow without affecting spending categories."
            />
          </View>
        ) : (
          shown.map(({ debt, balanceMinor }) => (
            ...
            >
              ...
              <Text className="mt-1 text-sm text-muted">
                {debtDueState(debt, balanceMinor).replace('_', ' ')}
              </Text>
              {debt.dueDate ? (
